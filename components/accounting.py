...
class Accounting(object):
    ...
    - **total_burst_time** (int):
        - Total burst time is dened as the amount of time a process is actually using the CPU.
    ...
    - **turnaround_time** (int):
        - Turnaround time is defined as the end-to-end time a process experiences for a single CPU
        burst. More specically, this starts with the arrival time in the ready queue through to
        when the process finishes all of its CPU bursts. As such, this measure includes context
        switch times.
    - **wait_time** (int):
        - Wait time is defined as the amount of time a process spends waiting to use the CPU, which
...
if __name__=='__main__':
    ...
    S = SystemAccounting()
    p = load_process_file(os.path.dirname(os.path.realpath(__file__))+'/../input_data/t.txt')
    processes = {}
    count = 0
